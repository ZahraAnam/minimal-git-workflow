---
description: Detect unpushed commits and let you choose what to do with them (push, squash, or leave as-is) before starting a session or applying a config change.
---

Run this any time you want to check for unpushed commits and resolve them
before continuing — e.g. before switching operational modes or starting a
fresh session.
...
user resolve it manually, then re-run this command if they want.
...
Whichever branch was taken, report the final state:
```bash
git log --oneline -3
git status --short
```
